...
# count is dynamic: one name per themes/*.theme palette file
want=$(ls "$THEME_ROOT"/*.theme | wc -l | tr -d ' ')
[ "$("$B" list --names | wc -l | tr -d ' ')" -eq "$want" ] || { echo "expected $want names"; exit 1; }
